...
func _ready() -> void:
	...
	_material.set_shader_parameter(&"ground_tex", TerrainTextures.build(noise_seed))
	_material.set_shader_parameter(&"tile_world_units", TerrainTextures.TILE_WORLD_UNITS)
	_material.set_shader_parameter(&"rock_tile_world_units", TerrainTextures.ROCK_TILE_WORLD_UNITS)
...
## Hands the authored farm to the ground shader. Called by main.gd once the plan is read,
## and again on live-reload. The plan only overrides the base ground; the slope rules
...
func apply_farm_plan(plan: FarmPlan) -> void:
	...
	# A missing or broken plan leaves this false, and the terrain renders its own rules
	# exactly as it did before there was a designer.
	_material.set_shader_parameter(&"plan_active", plan.loaded)
